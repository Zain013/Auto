...
async def handle_child_message(token, data):
    ...
    if state == "channel":
        ...
        if not chat_info:
            await send(token, chat_id, "❌ Channel not found. Send @username, a t.me channel link, numeric chat ID, or forward a channel message.", menu())
            return
        ok, msg = await add_channel_from_chat(token, chat_info)
        if ok:
            states.pop(state_key, None)
            await send(token, chat_id, msg, menu())
        else:
            await send(token, chat_id, "❌ " + msg, menu())
        return

    if state == "channel_select_on" or state == "channel_select_off":
        rows = channel_rows(token)
        chosen = next((r for r in rows if text == str(r["chat_id"]) or text == r["title"]), None)
        if not chosen:
            await send(token, chat_id, "❌ Select a channel from My Channels.", menu())
            return
        ...
        states.pop(state_key, None)
        if action:
            await send(token, chat_id, "🟢 AutoReq ON for this channel. Approving saved requests…", menu())
            await approve_saved(token)
        else:
            await send(token, chat_id, "🔴 AutoReq OFF for this channel.", menu())
        return
    ...
    if text == ADD_CHANNEL:
        ...
        await send(token, chat_id, "📺 Send channel @username / t.me link / numeric chat ID, OR forward any message from the channel here.\n\n⚠️ Bot must already be an admin with permission to approve join requests.", menu())
    elif text == MY_CHANNELS:
        rows = channel_rows(token)
        if not rows:
            await send(token, chat_id, "📺 No channels added yet.", menu())
            return
        lines = ["📺 My Channels\n"]
        for r in rows:
        ...
        rows = channel_rows(token)
        if not rows:
            await send(token, chat_id, "❌ Add a channel first.", menu())
            return
        if len(rows) == 1:
            chosen = rows[0]
            ...
            async with db_lock:
                db.execute("UPDATE channels SET autoreq=? WHERE token=? AND chat_id=?", (action, token, chosen["chat_id"]))
                db.commit()
            await send(token, chat_id, ("🟢 AutoReq ON" if action else "🔴 AutoReq OFF") + f" for {chosen['title']}", menu())
            if action:
                await approve_saved(token)
        else:
