...
async def handle_child_message(token, data):
    ...
    if text == "/start":
        if uid == OWNER_ID:
            ...
            await send(token, chat_id, "Use the channel join request to receive the configured message.")
        return
                ...
                for update in result.get("result", []):
                    ...
                    if request:
                        ...
                        if user_chat_id and child and child["welcome"]:
                            await api(session, token, "sendMessage", chat_id=user_chat_id, text=child["welcome"])
